...
def reverse_bounce_ball(collision_type):
    global ball_speed_x, ball_speed_y, ball_base_speed
...
player_width = 20   # NEW CHANGES (BY: KHALID):
player_height = 120
player_speed = 6
...
def main():
    running = True
    global khalid_score, kaamil_score, kaamil_colour, khalid_colour, ball_base_speed
    while running:
        ...
        if ball.x <= 0 or ball.x >= WIDTH:
            ...
            centre_ball()
            print(f"Kaamil: {kaamil_score} - Khalid: {khalid_score}")
        ...
        kaamil_score_text = font.render(f"Kaamil - {kaamil_score}", True, kaamil_colour)
        khalid_score_text = font.render(f"Khalid - {khalid_score}", True, khalid_colour)
        text_start = font.render(f"WELCOME TO THE GAME",)
        screen.blit(text_start, (WIDTH // 5, 20))

        screen.blit(kaamil_score_text, (WIDTH // 5, 20))
        screen.blit(khalid_score_text, (WIDTH * 3 // 5, 20))

        for event in pygame.event.get():
            if event.type == pygame.ball.colliderect:
                text_start = False
